...
test('extension UI exposes persistence, editing, layout, and data transfer controls', () => {
  ...
  for (const id of ['searchInput', 'bindDialog', 'importFileInput', 'keyboardContainer']) assert.match(html, new RegExp(`id=["']${id}["']`));
  for (const token of ['localStorage', 'JSON.stringify', 'JSON.parse', 'KEYBOARD_LAYOUTS']) assert.match(js, new RegExp(token.replace('.', '\\.' )));
  ...
  assert.match(read('content.js'), /web-app\.html\?extension=1/);
});
...
test('extension bundle contains an independent copy of the Web app', () => {
  ...
  assert.match(read('app.html'), /<script src="web-app\.js"><\/script>/);
});
